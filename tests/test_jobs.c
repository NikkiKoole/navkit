...
static void RunBenchmarks(void) {
    ...
    // Benchmark: FindNearestUnreservedItem
    ...
    for (int c = 0; c < numCounts; c++) {
        ...
        int numTests = 3;
        int numIterations = 1000;  // More iterations for measurable time
        
        // Benchmark NAIVE (use volatile to prevent optimization)
        ...
        // Benchmark SPATIAL (use volatile to prevent optimization)
        ...
        // Verify both return same result for all test positions
        bool allMatch = true;
        for (int t = 0; t < numTests; t++) {
            ...
            if (naiveResult != spatialResult) {
                // Check if they're actually different distances or just different items at same distance
                ...
                // Allow if same distance (tie-breaker difference is acceptable)
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (argv[i][0] == '-' && argv[i][1] == 'b') benchmarks = true;
    }
    if (!verbose) {
