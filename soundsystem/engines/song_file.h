...
static void _sf_writePatch(FILE *f, const char *section, const SynthPatch *p) {
    ...
    _sf_writeFloat(f, "stifkarpStrikePos", p->p_stifkarpStrikePos);
    _sf_writeFloat(f, "stifkarpBodyMix", p->p_stifkarpBodyMix);
    ...
    _sf_writeInt(f, "vfPhoneme", p->p_vfPhoneme);
    _sf_writeInt(f, "vfPhonemeTarget", p->p_vfPhonemeTarget);
    _sf_writeFloat(f, "vfMorphRate", p->p_vfMorphRate);
    _sf_writeFloat(f, "vfAspiration", p->p_vfAspiration);
    _sf_writeFloat(f, "vfOpenQuotient", p->p_vfOpenQuotient);
    _sf_writeFloat(f, "vfSpectralTilt", p->p_vfSpectralTilt);
    _sf_writeFloat(f, "vfFormantShift", p->p_vfFormantShift);
    _sf_writeFloat(f, "vfVibratoDepth", p->p_vfVibratoDepth);
    _sf_writeFloat(f, "vfVibratoRate", p->p_vfVibratoRate);
    _sf_writeInt(f, "vfConsonant", p->p_vfConsonant);
...
static void _sf_writePattern(FILE *f, int idx, const Pattern *p) {
    ...
    if (p->trackType == TRACK_DRUM) {
        ...
        // Melody/sampler events — write all voices per step
        for (int s = 0; s < p->length; s++) {
            ...
            for (int v = 0; v < sv->noteCount; v++) {
                ...
                fprintf(f, "m step=%d note=%s vel=%.3g gate=%d",
                        s, noteName, (double)velU8ToFloat(sn->velocity), (int)sn->gate);
...
static void _sf_applyPatchKV(SynthPatch *p, const char *key, const char *val) {
    ...
    else if (strcmp(key, "stifkarpStrikePos") == 0) p->p_stifkarpStrikePos = _sf_parseFloat(val);
    else if (strcmp(key, "stifkarpBodyMix") == 0) p->p_stifkarpBodyMix = _sf_parseFloat(val);
    ...
    else if (strcmp(key, "vfPhoneme") == 0) p->p_vfPhoneme = _sf_parseInt(val);
    else if (strcmp(key, "vfPhonemeTarget") == 0) p->p_vfPhonemeTarget = _sf_parseInt(val);
    else if (strcmp(key, "vfMorphRate") == 0) p->p_vfMorphRate = _sf_parseFloat(val);
    else if (strcmp(key, "vfAspiration") == 0) p->p_vfAspiration = _sf_parseFloat(val);
    else if (strcmp(key, "vfOpenQuotient") == 0) p->p_vfOpenQuotient = _sf_parseFloat(val);
    else if (strcmp(key, "vfSpectralTilt") == 0) p->p_vfSpectralTilt = _sf_parseFloat(val);
    else if (strcmp(key, "vfFormantShift") == 0) p->p_vfFormantShift = _sf_parseFloat(val);
    else if (strcmp(key, "vfVibratoDepth") == 0) p->p_vfVibratoDepth = _sf_parseFloat(val);
    else if (strcmp(key, "vfVibratoRate") == 0) p->p_vfVibratoRate = _sf_parseFloat(val);
    else if (strcmp(key, "vfConsonant") == 0) p->p_vfConsonant = _sf_parseInt(val);
        ...
        case _SF_SEC_PATTERN:
            if (subIndex >= 0 && subIndex < SEQ_NUM_PATTERNS) {
                ...
                if (strcmp(key, "length") == 0) {
                    int l = atoi(val);
                    p->length = (l > 0 && l <= SEQ_MAX_STEPS) ? l : SEQ_MAX_STEPS;
                }
