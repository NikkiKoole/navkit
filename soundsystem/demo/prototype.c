...
static void _oldPresets(void) {
    ...
    instrumentPresets[12].patch.p_pulseWidth = 0.15f;      // Thin = nasal/reedy
    ...
    instrumentPresets[12].patch.p_vibratoDepth = 0.3f;     // Wobbly
    instrumentPresets[12].patch.p_filterCutoff = 0.9f;     // Bright
    ...
    instrumentPresets[13].patch.p_filterCutoff = 0.25f;    // Low = muffled
    instrumentPresets[13].patch.p_filterResonance = 0.6f;  // High = chirpy "bwa"
    instrumentPresets[13].patch.p_filterEnvAmt = 0.4f;     // Opens on attack
    instrumentPresets[13].patch.p_filterEnvDecay = 0.08f;  // Quick chirp
    
    // Piku FM Bell - Bright glassy toy bell
    instrumentPresets[14].name = "Piku Bell";
    instrumentPresets[14].patch.p_waveType = WAVE_FM;
    instrumentPresets[14].patch.p_fmModRatio = 3.5f;       // Metallic ratio
    instrumentPresets[14].patch.p_fmModIndex = 2.5f;       // Sparkly
    ...
    instrumentPresets[16].patch.p_additiveInharmonicity = 0.01f;  // Toy quality
    ...
    // Jangly, chorus-drenched, detuned, tape-warbled lo-fi vibes
    // ========================================================================
    
    // Mac Jangle - Chorus-y jangly guitar (Chamber of Reflection vibes)
    instrumentPresets[18].name = "Mac Jangle";
    instrumentPresets[18].patch.p_waveType = WAVE_PLUCK;
    instrumentPresets[18].patch.p_pluckBrightness = 0.5f;   // Warm, not too bright
    instrumentPresets[18].patch.p_pluckDamping = 0.994f;    // Medium sustain
    instrumentPresets[18].patch.p_filterCutoff = 0.6f;      // Rolled off highs
    instrumentPresets[18].patch.p_filterResonance = 0.15f;
    instrumentPresets[18].patch.p_vibratoRate = 4.5f;       // Slow wobbly vibrato
    instrumentPresets[18].patch.p_vibratoDepth = 0.15f;     // Subtle pitch drift
    
    // Mac Juno - That classic Juno-60 pad sound (slow attack, chorus, detuned)
    instrumentPresets[19].name = "Mac Juno";
    instrumentPresets[19].patch.p_waveType = WAVE_SAW;
    instrumentPresets[19].patch.p_attack = 0.15f;           // Slow, lazy attack
    instrumentPresets[19].patch.p_decay = 0.3f;
    instrumentPresets[19].patch.p_sustain = 0.7f;
    instrumentPresets[19].patch.p_release = 0.6f;           // Long release
    instrumentPresets[19].patch.p_filterCutoff = 0.45f;     // Warm, muffled
    instrumentPresets[19].patch.p_filterResonance = 0.1f;
    instrumentPresets[19].patch.p_unisonCount = 2;          // Detuned unison
    instrumentPresets[19].patch.p_unisonDetune = 12.0f;     // Noticeable detune
    instrumentPresets[19].patch.p_vibratoRate = 0.3f;       // Very slow drift
    ...
    instrumentPresets[20].patch.p_filterCutoff = 0.3f;      // Dark, round
    instrumentPresets[20].patch.p_filterResonance = 0.25f;  // Slight honk
    instrumentPresets[20].patch.p_filterEnvAmt = 0.15f;     // Gentle pluck
    instrumentPresets[20].patch.p_filterEnvDecay = 0.2f;
    instrumentPresets[20].patch.p_vibratoRate = 5.0f;
    instrumentPresets[20].patch.p_vibratoDepth = 0.05f;     // Tiny wobble
    
    // Mac Keys - Electric piano, slightly out of tune (Freaking Out vibes)
    instrumentPresets[21].name = "Mac Keys";
    instrumentPresets[21].patch.p_waveType = WAVE_FM;
    instrumentPresets[21].patch.p_fmModRatio = 1.0f;        // EP-style ratio
    instrumentPresets[21].patch.p_fmModIndex = 1.2f;        // Gentle FM
    ...
    instrumentPresets[21].patch.p_vibratoRate = 5.5f;       // Tremolo-ish
    instrumentPresets[21].patch.p_vibratoDepth = 0.12f;     // Wobbly
    
    // Mac Vox - Dreamy vocal pad (ooh/aah with drift)
    ...
    instrumentPresets[22].patch.p_attack = 0.2f;            // Slow fade in
    ...
    instrumentPresets[22].patch.p_vibratoDepth = 0.2f;      // Drunken vibrato
    
    // Mac Vibes - Detuned vibraphone (that lo-fi mallet sound)
    ...
    instrumentPresets[23].patch.p_malletTremolo = 0.6f;     // Motor on
    instrumentPresets[23].patch.p_malletTremoloRate = 4.5f; // Slow tremolo
...
static void songFileFromEngine(SongFileData *d) {
    ...
    for (int i = 0; i < SEQ_V2_MAX_TRACKS; i++) d->sfTrackVolume[i] = seq.trackVolume[i];
    // Effects
    d->sfEffects = fx;
    d->sfDubLoop = dubLoop;
...
static void songFileToEngine(const SongFileData *d) {
    ...
    for (int i = 0; i < SEQ_V2_MAX_TRACKS; i++) seq.trackVolume[i] = d->sfTrackVolume[i];
    // Effects
    fx = d->sfEffects;
    dubLoop = d->sfDubLoop;
...
static void updateDrumTrackName(int track) {
    if (track >= 0 && track < SEQ_DRUM_TRACKS) {
        seq.trackNames[track] = drumTypeShortNames[drumTrackSound[track]];
    }
}
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (SectionHeader(col1.x, col1.y, "Wave", &showWaveColumn)) {
            ...
            ui_col_sublabel(&col1, "Preset:", ORANGE);
            {
                static const char* presetNames[NUM_INSTRUMENT_PRESETS];
                for (int i = 0; i < NUM_INSTRUMENT_PRESETS; i++) {
                    presetNames[i] = instrumentPresets[i].name;
                }
                int prevPreset = currentPresetIndex;
                ui_col_cycle(&col1, "Load", presetNames, NUM_INSTRUMENT_PRESETS, &currentPresetIndex);
                if (currentPresetIndex != prevPreset) {
                    patches[selectedPatch] = instrumentPresets[currentPresetIndex].patch;
                    selectedWave = patches[selectedPatch].p_waveType;
                }
            }
            ui_col_space(&col1, 4);
        ...
        if (SectionHeader(col5.x, col5.y, "Effects", &showEffectsColumn)) {
            ...
            ui_col_sublabel(&col5, "Chorus:", ORANGE);
            ui_col_toggle(&col5, "On", &fx.chorusEnabled);
            ui_col_float(&col5, "Rate", &fx.chorusRate, 0.1f, 0.1f, 5.0f);
            ui_col_float(&col5, "Depth", &fx.chorusDepth, 0.05f, 0.0f, 1.0f);
            ...
            // Pikuniku-style effects presets (synth presets are in Load cycle)
            ui_col_sublabel(&col5, "Piku FX:", PINK);
            if (ui_col_button(&col5, "Clean")) { pikuEffectsClean(); }
            if (ui_col_button(&col5, "9-Bit")) { pikuEffects9Bit(); }
            if (ui_col_button(&col5, "Wobbly")) { pikuEffectsWobbly(); }
            if (ui_col_button(&col5, "Toy")) { pikuEffectsToy(); }
            ui_col_space(&col5, 4);
            
            // Mac DeMarco-style effects presets
            ui_col_sublabel(&col5, "Mac FX:", SKYBLUE);
            if (ui_col_button(&col5, "Tape+Chorus")) { macEffectsTapeChorus(); }
            if (ui_col_button(&col5, "Chorus")) { macEffectsChorus(); }
            if (ui_col_button(&col5, "Lo-Fi")) { macEffectsLoFi(); }
        }
        ...
        // === SEQUENCER GRID (Drums + Melodic) ===
        {
            ...
            // === PATTERN BAR ===
            {
                ...
                for (int i = 0; i < SEQ_NUM_PATTERNS; i++) {
                    ...
                    for (int t = 0; t < SEQ_MELODY_TRACKS && !hasContent; t++) {
                        for (int s = 0; s < SEQ_MAX_STEPS && !hasContent; s++) {
                            if (patGetNote(&seq.patterns[i], SEQ_DRUM_TRACKS + t, s) != SEQ_NOTE_OFF) hasContent = true;
                        }
                    }
            ...
            for (int track = 0; track < SEQ_DRUM_TRACKS; track++) {
                ...
                DrawTextShadow(seq.trackNames[track], gridX, y + 3, 12, labelColor);
                        ...
                        if (mouseClicked) {
                            if (isActive) {
                                ...
                                if (seq.trackNoteOn[track]) {
                                    seq.trackNoteOn[track](0, patGetDrumVel(p, track, step), 0, pitchMod, false, false);
                                }
                            }
                        }
            ...
            for (int track = 0; track < SEQ_MELODY_TRACKS; track++) {
                ...
                DrawTextShadow(seq.trackNames[SEQ_DRUM_TRACKS + track], gridX, y + 3, 12, labelColor);
                ...
                for (int step = 0; step < trackLen; step++) {
                    ...
                    int note = patGetNote(p, SEQ_DRUM_TRACKS + track, step);
                    bool hasNote = (note != SEQ_NOTE_OFF);
                    bool isCurrent = (step == seq.trackStep[SEQ_DRUM_TRACKS + track]) && seq.playing;
                    bool isHovered = CheckCollisionPointRec(mouse, cell);
                    bool isSelected = selectedIsMelody && selectedTrack == track && selectedStep == step;
                    bool hasProb = hasNote && patGetNoteProb(p, SEQ_DRUM_TRACKS + track, step) < 1.0f;
                    bool hasCond = hasNote && patGetNoteCond(p, SEQ_DRUM_TRACKS + track, step) != COND_ALWAYS;
                    bool hasSlide = hasNote && patGetNoteSlide(p, SEQ_DRUM_TRACKS + track, step);
                    bool hasAccent = hasNote && patGetNoteAccent(p, SEQ_DRUM_TRACKS + track, step);

                    Color bgColor = (step / 4) % 2 == 0 ? (Color){35, 38, 45, 255} : (Color){28, 30, 38, 255};

                    Color cellColor = bgColor;
                    if (hasNote) {
                        float vel = patGetNoteVel(p, SEQ_DRUM_TRACKS + track, step);
                    ...
                    if (hasNote && seqHasPLocks(p, SEQ_DRUM_TRACKS + track, step)) {
                        int dx = x + cellW - 8;
                        int dy = y + cellH - 8;
                        DrawTriangle(
                    ...
                    if (isHovered && fabsf(mouseWheel) > 0.1f) {
                        if (hasNote) {
                            ...
                            patSetNotePitch(p, SEQ_DRUM_TRACKS + track, step, (int)clampf(note + delta, 24, 96));  // C1 to C7
                        }
                    }
                    ...
                    if (isHovered) {
                        if (mouseClicked) {
                            ...
                            if (!hasNote) {
                                ...
                                patSetNote(p, SEQ_DRUM_TRACKS + track, step, defaultNotes[track], 0.8f, 1);
                            }
                            ui_consume_click();
                        }
                        if (rightClicked && hasNote) {
                            patClearNote(p, SEQ_DRUM_TRACKS + track, step);
                            if (selectedIsMelody && selectedTrack == track && selectedStep == step) {
                                selectedTrack = -1;
                                selectedStep = -1;
                            }
            ...
            bool showMelodyInspector = selectedIsMelody && selectedTrack >= 0 && selectedStep >= 0
                                       && patGetNote(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep) != SEQ_NOTE_OFF;
            ...
            if (showDrumInspector) {
                ...
                DrawTextShadow(TextFormat("Step %d - %s", selectedStep + 1, seq.trackNames[selectedTrack]), 
                ...
                {
                    float vel = patGetDrumVel(p, selectedTrack, selectedStep);
                    DraggableFloat(inspX + 10, row1Y, "Vel", &vel, 0.02f, 0.0f, 1.0f);
                    patSetDrumVel(p, selectedTrack, selectedStep, vel);
                }
                ...
                {
                    float prob = patGetDrumProb(p, selectedTrack, selectedStep);
                    DraggableFloat(inspX + 10 + colSpacing * 2, row1Y, "Prob", &prob, 0.02f, 0.0f, 1.0f);
                    patSetDrumProb(p, selectedTrack, selectedStep, prob);
                }
            ...
            if (showMelodyInspector) {
                ...
                int note = patGetNote(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                DrawTextShadow(TextFormat("Step %d - %s [%s]", selectedStep + 1, seq.trackNames[SEQ_DRUM_TRACKS + selectedTrack], seqNoteName(note)), 
                ...
                // Note (as int for simplicity)
                {
                    ...
                    if (noteHovered) {
                        ...
                        if (fabsf(wheel) > 0.1f) {
                            ...
                            patSetNotePitch(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, (int)clampf(note + delta, 24, 96));
                        }
                    }
                }

                {
                    float vel = patGetNoteVel(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                    DraggableFloat(inspX + 10 + colSpacing, row1Y, "Vel", &vel, 0.02f, 0.0f, 1.0f);
                    patSetNoteVel(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, vel);
                }
                ...
                // Gate length
                {
                    ...
                    int gate = patGetNoteGate(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                    DrawTextShadow(TextFormat("%d", gate), gateX + 48, row1Y, 10, WHITE);
                    if (gateHovered && mouseClicked) {
                        patSetNoteGate(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, (gate % 16) + 1);
                        ui_consume_click();
                    }
                    if (gateHovered && rightClicked) {
                        int newGate = gate - 1;
                        if (newGate < 1) newGate = 16;
                        patSetNoteGate(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, newGate);
                        ui_consume_click();
                    }
                }

                {
                    float prob = patGetNoteProb(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                    DraggableFloat(inspX + 10 + colSpacing * 3, row1Y, "Prob", &prob, 0.02f, 0.0f, 1.0f);
                    patSetNoteProb(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, prob);
                }
                ...
                // Condition
                {
                    ...
                    DrawRectangleRec(condRect, condHovered ? (Color){60, 60, 70, 255} : (Color){45, 45, 55, 255});
                    DrawRectangleLinesEx(condRect, 1, condHovered ? YELLOW : (Color){80, 80, 80, 255});
                    int cond = patGetNoteCond(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                    DrawTextShadow(conditionNames[cond], condX + 44, row1Y, 10, WHITE);
                    if (condHovered && mouseClicked) {
                        patSetNoteCond(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, (cond + 1) % COND_COUNT);
                        ui_consume_click();
                    }
                    if (condHovered && rightClicked) {
                        patSetNoteCond(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, (cond - 1 + COND_COUNT) % COND_COUNT);
                        ui_consume_click();
                    }
                }
                ...
                // 303-style Slide toggle
                {
                    ...
                    bool hasSlide = patGetNoteSlide(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                    ...
                    if (slideHovered && mouseClicked) {
                        patSetNoteFlags(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, !hasSlide, patGetNoteAccent(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep));
                        ui_consume_click();
                    }
                }
                ...
                // 303-style Accent toggle
                {
                    ...
                    bool hasAccent = patGetNoteAccent(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                    ...
                    if (accentHovered && mouseClicked) {
                        patSetNoteFlags(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, patGetNoteSlide(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep), !hasAccent);
                        ui_consume_click();
                    }
                }
                ...
                // Sustain (extra hold steps after gate expires)
                {
                    ...
                    int susVal = patGetNoteSustain(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
                    ...
                    if (susHovered && mouseClicked) {
                        patSetNoteSustain(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, (susVal + 1) % 17);
                        ui_consume_click();
                    }
                    if (susHovered && rightClicked) {
                        patSetNoteSustain(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, (susVal - 1 + 17) % 17);
                        ui_consume_click();
                    }
                    if (susHovered && mouseWheel != 0) {
                        ...
                        patSetNoteSustain(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep, newVal);
                    }
                }
                ...
                // P-lock: Volume
                {
                    ...
                    float vol = seqGetPLock(p, absTrack, selectedStep, PLOCK_VOLUME, -1.0f);
                    bool isLocked = (vol >= 0.0f);
                    if (!isLocked) vol = patGetNoteVel(p, SEQ_DRUM_TRACKS + selectedTrack, selectedStep);
